...
public:
  ...
  PrefixLB(CkMigrateMessage *m);
  void GossipLoadInfo(int, int, int, int[], double[]);
  void AvgLoadReduction(double x);
  void MaxLoadReductionBeforeLB(double init_max_load);
  void receivedPrefixSum(double sum);
  void MaxNacks(int max_nack);
  void FoundPrefix();
  void expected(int migrates);
  void SendAfterBarrier();
  void SendAfterBarrier2();
  void run();

private:
  CProxy_PrefixLB thisProxy;
  ...
  double kTargetTransferThreshold;
  ...
  void Setup();
  void Cleanup();
  void PackAllMigrateMsgs();
  void DoneWithLBPhase(double imb);
